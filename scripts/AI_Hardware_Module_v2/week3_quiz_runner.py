...
quiz_path = 'H:\\杨霞博士期间计划书\\主2\\子2.1\\AI_Hardware_Module_v2\\quiz_library\\week3_ethics_quiz.json'
if os.path.exists(quiz_path):
    with open(quiz_path, 'r', encoding='utf-8') as f:
        quiz_data = json.load(f)
    questions = quiz_data.get('questions', [])  # Assume JSON has 'questions' list
    print("Quiz loaded: Week 3 Ethics Quiz + Quantum OS Trends")
else:
...
for q in questions:
    ...
    if options:
        for opt in options:  # Print options directly (they include A. etc.)
            print(opt)
...
print("Quiz complete. Results saved to logs/week3_quiz_results.txt")

# Save to logs (detailed file creation)
logs_path = 'H:\\杨霞博士期间计划书\\主2\\子2.1\\AI_Hardware_Module_v2\\logs\\week3_quiz_results.txt'
os.makedirs(os.path.dirname(logs_path), exist_ok=True)
with open(logs_path, 'w', encoding='utf-8') as f:
    f.write(f"Week 3 Quiz Results:\nScore: {score}/{total}\n")
    f.write('\n'.join(results))
