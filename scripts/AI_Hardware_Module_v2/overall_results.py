...
# 加载合并数据（如果不存在，使用单独周文件或模拟）
try:
    merged_df = pd.read_excel(r'H:\杨霞博士期间计划书\主2\子2.1\AI_Hardware_Module_v2\logs\merged_weekly_results.xlsx')
...
print(df_to_markdown(overall_table, index=False))  # 输出 Markdown 表到控制台，或保存到文件

# 生成兴趣趋势图（假设 Pre_Score/Post_Score 为兴趣得分）
plt.rcParams['font.sans-serif'] = ['AR PL KaitiM GB'] + plt.rcParams['font.sans-serif']  # 使用支持中文的字体
plt.rcParams['axes.unicode_minus'] = False  # 避免负号问题

plt.figure(figsize=(10, 6))
plt.plot(weekly_pre.index, weekly_pre.values, marker='o', label='Pre Interest (前兴趣得分)')
plt.plot(weekly_post.index, weekly_post.values, marker='o', label='Post Interest (后兴趣得分)')
plt.xlabel('周次 (Week)')
plt.ylabel('平均得分 (Average Score)')
plt.title('8周兴趣趋势图 (Interest Trend Over 8 Weeks)')
plt.legend()
plt.grid(True)
plt.xticks(range(1, 9))
plt.savefig('interest_trend.png')
print('兴趣趋势图已保存到 interest_trend.png')